...
function diplayMenuItems(menuItems) {
    ...
    menuItems.forEach(function (item) {
        ...
        <img src=${item.img} alt=${item.title} class="photo" />
        <div class="item-info">
        <p><button class="button" onClick=ddToCard(${item.title} )>Adauga in cos</botton></p>
            <header>
                <h4>${item.title}</h4>
                <h4 class="price">${item.price} RON</h4>
            </header>
            <p class="item-text"> ${item.desc}</p>
            <p><buttom> Adauga in cos</buttom></p>
        </div>
        </article>`;
...
function counterClick(btn) {
    ...
    function addToCart(price){
        ...
        myCart.valoare_initiala = +(myCart.innerText || myCart.textContent);
        myCart.innerHTML = valoare_initiala.innerHTML + price;
